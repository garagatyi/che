...
import org.eclipse.che.api.workspace.server.sidecartooling.model.CheContainer;
import org.eclipse.che.api.workspace.server.sidecartooling.model.CheContainerPort;
import org.eclipse.che.api.workspace.server.sidecartooling.model.CheFeature;
import org.eclipse.che.api.workspace.server.sidecartooling.model.ChePlugin;
import org.eclipse.che.api.workspace.server.sidecartooling.model.ChePluginEndpoint;
import org.eclipse.che.api.workspace.server.sidecartooling.model.ChePluginParameter;
import org.eclipse.che.api.workspace.server.sidecartooling.model.ChePluginReference;
import org.eclipse.che.api.workspace.server.sidecartooling.model.EnvVar;
...
/**
 * Fetches {@link ChePlugin} objects corresponding to attributes of a workspace.
 *
 * @author Oleksander Garagatyi
 */
@Beta
public class ChePluginsRetriever {

  private static final Logger LOG = LoggerFactory.getLogger(ChePluginsRetriever.class);
  ...
  @Inject
  public ChePluginsRetriever(@Nullable @Named("che.workspace.feature.api") String featureApi) {
    if (featureApi == null) {
      LOG.info(
          "Che plugins flow is disabled - Feature API endpoint property 'che.workspace.feature.api' is not configured");
  ...
  /**
   * Gets {@link ChePlugin} list from provided workspace attributes, fetches corresponding objects
   * from Feature API and returns list of {@link ChePlugin} needed to provide features functionality
   * in a workspace.
   *
   ...
   * @throws InfrastructureException when features list contains invalid entries or ChePlugin
  ...
  private Collection<Pair<String, String>> parseFeatures(String[] features)
    ...
    for (String feature : features) {
      ...
      if (featuresNameVersion.containsKey(key)) {
        throw new InfrastructureException(
            format("Invalid Che plugins configuration: feature %s is duplicated", key));
      }
